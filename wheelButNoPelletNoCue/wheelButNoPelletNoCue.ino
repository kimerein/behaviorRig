...
class StepperDisk
{
  ...
  public:
    ...
    void Update(long currStepSize, long waitToTurn, boolean trialStarting, int doTurn)
    {
      if (doTurn == 0) {
        return
      }
      // only turn wheel if doTurn == 1
...
class FlashOnce
{
    ...
    long OffTimeVary; // choose a random delay between 0 and OffTimeVary to add to OffTime
    long OffSum; // sum of OffTime and random delay for this trial
  ...
  public:
    FlashOnce(int pin, long on, long off, long offvary, int eventLabel)
    {
      ledPin = pin;
    ...
    void Update(boolean trialStarting, int doFlash)
    {
      ...
      if (trialStarting == true)
      {
        ...
        // choose a random delay to add to OffTime
        OffSum = OffTime + random(0,OffTimeVary);
      }
      ...
      else if ((ledState == HIGH) && (currentMillis - previousMillis >= OffSum) && (didOnce == false)) // Haven't yet turned LED on
      {
        ...
        if (doFlash == 1)
        {
          ...
          outString = serialStringOut(">SF>", outString);
        }
        outString = serialStringOut(String(millis()), outString);
        outString = serialStringOut("\r\n", outString);
        //      outString = serialStringOut(">", outString);
...
const long cueDelayVary = 1000; // add a random delay less than or equal to this time after cueDelay (note that cueDelayVary + cueDelay must be < loaderDelay)
...
const long minITI = 8000;
const long maxITI = 12000;
const long pelletsDelay = 0; // in ms
const long loaderDelay = 4000; // in ms
int loadNow = {1, 0, 0};
int cueNow = {0, 0, 1}; 
...
FlashOnce cue(cuePin, cueDuration, cueDelay, cueDelayVary, cueWriteCode);
...
void loop()
{
  ...
  loader.Update(1, loaderDelay, trialIsStarting, loadNow[trialCount]);
  pellets.Update(20, pelletsDelay, trialIsStarting, 1);
  cue.Update(trialIsStarting, cueNow[trialCount]);

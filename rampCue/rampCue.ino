...
class FlashOnce
{
    ...
    unsigned long rampingMillis; // stores last time duty cycle was updated
  ...
  public:
    ...
    void UpdateDutyCycle(boolean trialStarting, boolean turnCueOn)
    {
      ...
      {
        // begin ramp
        // every 20 ms, increase duty cycle by 5
        // reset ramp variables
        ledState = HIGH; // Turn it on
        dutyCycle = 5;
        previousMillis = currentMillis; // Remember the time
        rampingMillis = currentMillis; // Remember the last time duty cycle was updated
        analogWrite(ledPin, dutyCycle); // Update the actual LED
        String outString = "";
        outString = serialStringOut(String(writeCode), outString);
        outString = serialStringOut(">S>", outString);
        outString = serialStringOut(String(millis()), outString);
      ...
      else if ((ledState == HIGH) && (currentMillis - previousMillis < OnTime) && (currentMillis - rampingMillis >= 20)) // Increase duty cycle
      {
...
const long cueDuration = 1000; // cue on duration in ms
...
const long optoDelay = 1770; // delay from trial start until opto turns on in ms
...
void loop()
{
  ...
  cue.UpdateDutyCycle(trialIsStarting, pelletToMouse);

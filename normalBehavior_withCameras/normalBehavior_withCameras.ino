...
const long cameraTriggerDur = 10; // ms duration of camera digital output trigger
const long cameraTriggerOffTime = 100; // ms duration of time between camera triggers
...
void loop()
{
  ...
  if ((trialState == LOW) && ((currMs - prevMs) > ITI))
  {
    ...
    if (trialCount >= nTrialsBetweenCues)
    {
      ...
      // Julia modify next line -- start at 30, increase by increments of 5
      if (randval > 40) {

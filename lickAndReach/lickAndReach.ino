...
class FlashRandom
{
    ...
    long OffTime; // milliseconds of off-time
    int ledState; // led state
    long minDelay; // minimum time delay between random LED transitions, in milliseconds
    long maxDelay; // maximum time delay between random LED transitions, in milliseconds
    unsigned long previousMillis; // stores last time LED was updated
    int writeCode;
  ...
  public:
    FlashRandom(int pin, int unconnectedPin, long minRand, long maxRand, int eventLabel, long off, int distractorP, long cueDuration)
    {
      ledPin = pin;
      pinMode(ledPin, OUTPUT);
      minDelay = minRand;
    ...
    void ChangeDuration(long minRand, long maxRand, long cueDuration)
    {
      cueOn = cueDuration;
      minDelay = minRand;
      maxDelay = maxRand;
...
// Declare constants
const unsigned int loaderN = 1;
const unsigned int pelletsN = 2;
const uint8_t motorShieldAddress = 0x60;
const unsigned int stepsForStepper = 200;
const unsigned int cuePin = 3; // digital output pin that controls cue
const unsigned int optoPin = 6; // digital output pin that controls opto for silencing 
const unsigned int solenoidPin = 5; // digital output pin that controls solenoid
const unsigned long cueDuration = 250; // cue on duration in ms
const unsigned long cueDelay = 1500; // delay from trial start until cue turns on in ms
const unsigned long optoDuration = 1000; // opto on duration in ms
const unsigned long optoDelay = 1520; // delay from trial start until opto turns on in ms
const unsigned int distractorPin = 2; // digital output pin that controls distractor
const unsigned int emptyPin = 4; // an analog input pin that is not connected (for random seed initialization)
const unsigned int randomMin = 1000; // in ms
const unsigned int randomMax = 10000; // in ms
// const unsigned long probabilityDistractor = 0.02; // probability that distractor will be on versus off
const unsigned int beginTrialWrite = 0;
const unsigned int loaderWriteCode = 1;
const unsigned int pelletsWriteCode = 2;
const unsigned int optoWriteCode = 3;
const unsigned int cueWriteCode = 4;
const unsigned int distractorWriteCode = 5;
const unsigned int interlockWriteCode = 6;
const unsigned int minITI = 11500;
const unsigned int maxITI = 12000;
const unsigned int pelletsDelay = 0; // in ms
const unsigned int loaderDelay = 4000; // in ms
const int probOfOpto = 45; // turn on opto in this percent of trials
const unsigned int nTrialsLiquidBlock = 10; // liquid block lasts for 6 min, about 4 cues per block
const unsigned int nTrialsSolidBlock = 9; // solid block lasts about 3 min
// const unsigned long interlockUpdate = 5000; // in ms, how often to write interlock value
const unsigned int solenoidOnDur=1; // in ms
...
int optoThisTime = 0;
boolean inLiquidBlock = false; // if true, in block primarily dispensing liquid, else in block primarily for pellet reaching

// probabilityDistractor for FlashRandom should match ratio of cueDuration to expected value of [randomMin, randomMax]
FlashRandom distractor(distractorPin, emptyPin, randomMin, randomMax, distractorWriteCode, cueDelay + cueDuration, 0, cueDuration);
...
FlashRandom solenoid(solenoidPin, emptyPin, 20000, 60000, 10, cueDelay + cueDuration, 0, solenoidOnDur);
...
void loop()
{
  ...
  if ((trialState == LOW) && ((currMs - prevMs) > ITI))
  {
    ...
    if (inLiquidBlock == true)
    {
      // in block primarily dispensing liquid
      distractor.ChangeDuration(randomMin, randomMax, 50); // distractor flash is shorter duration and more frequent
      solenoid.ChangeDuration(5900, 6000, solenoidOnDur); // dispense liquid every 6 seconds
      // solenoid.ChangeDuration(17900, 18000, solenoidOnDur); // dispense liquid every 6 seconds
      ITI = random(2.5*minITI, 2.5*maxITI); // pellets are presented less frequently
      // switch blocks?
      if (nTrials >= nTrialsLiquidBlock)
      {
      ...
      // in block with pellets
      distractor.ChangeDuration(randomMin, randomMax, cueDuration); // distractor flash is slightly longer and less frequent
      solenoid.ChangeDuration(60000, 120000, solenoidOnDur); // dispense liquid only once every 1-2 min
      ITI = random(minITI, maxITI); // pellets are presented more frequently
      // switch blocks?
      if (nTrials >= nTrialsSolidBlock)
      {
        // switch to liquid block
        nTrials = 0;
        inLiquidBlock = true;
        solenoid.ChangeDuration(5900, 6000, solenoidOnDur); // dispense liquid every 6 seconds
        // solenoid.ChangeDuration(17900, 18000, solenoidOnDur); // dispense liquid every 6 seconds
  ...
  solenoid.Update(trialIsStarting);
  // interlock.Update();
}
